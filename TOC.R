...
#Function to calculate TOC table
#Inputs: (See doc)
#mask is a raster image
#boolean is a raster this is the reference image...
#index is a raster image
#Output:
#  
...
index <- index*mask #this is directly in memory but might be ok, we coud use other fuction...
#index<- mask(index,mask)
boolean <- boolean*mask

boolval <- getValues(boolean) #not a good idea to  read directly but I understand this works well for now and we can  improve this!
...
func_logical3   <- function(v1,v2){
    r1  <- sum(v1 & v2) #is this from bit package?
    r2  <- sum(v1 & !v2)
    return(c(r1, r2))
}


## Ok this would make a nice function for non raster/spatial index and boolean  cases...
#added indent to help follow the code
if(!ranking){ #? is this if(ranking==TRUE){
  ...
  #we can make this a reclass function...
  for (j in 2:(nrow(res))){
    i <- newThres[j]
    zeroIndVal[which(indval > i)]  <- 1

    xb <- as.bit(zeroIndVal) #why are we using bit?? reduce mem usage ? we can read line by line...
    yb <- as.bit(boolval)
    crsstb <- func_logical3(xb,yb) #we can  use crosstab from raster!!!
...
names(tocd) <- c("Threshold", "A", "HitsRate", "B", "falseAlarmsRate") #ok dataframe
...
#Changed to:
units <- strsplit(strsplit(CRSargs(CRS(proj4string(index))), "+units=")[[1]][2], " ")[[1]][1]
#how about:
